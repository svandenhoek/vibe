...
function main {
    ...
    cd "$BASE_PATH"
	tar --exclude '.*' --exclude 'disgenet_full_tdb' --exclude 'disgenet_mini_tdb' --exclude 'disgenet_mini_tdb_no_ontology' -czvf "test_resources_$(date "+%Y-%m-%d").tar.gz" "src/test/resources/"*
	cd "$start_pwd"
}
